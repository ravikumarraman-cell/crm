    ...
    <main className="app-shell">
      <aside className="rail global-rail" aria-label="CRM navigation">
        ...
        <nav className="crm-nav">
          ...
          <div className={mobileCallsMenuOpen ? 'calls-module mobile-calls-menu-open' : 'calls-module'}>
            <button className="crm-nav-item active-module" onClick={() => { setView('workspace'); setMobileCallsMenuOpen((open) => !open) }} aria-controls="calls-subnav"><PhoneCall size={18} /><span>Calls</span></button>
      ...
      <section className="workspace" id="workspace">
        ...
        {view === 'overview' && <section className="operations-view overview-view" aria-labelledby="overview-title">
          ...
          <section className="crm-context-strip" aria-label="Current CRM context"><div><p className="eyebrow">CRM CONTEXT · NEXT RELATIONSHIP</p><strong>{selectedContact.name} <span className="dot">•</span> {selectedContact.company}</strong><p>{selectedContact.reason}. {contactTasks.length ? `${contactTasks.length} open commitment${contactTasks.length === 1 ? '' : 's'} owned by you.` : 'No open commitments.'}</p></div><button className="outline-button" onClick={() => setView('record')}>Open relationship record</button></section>
